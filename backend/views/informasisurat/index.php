...
foreach ($informasisurat as $value1) :
endforeach;
?>
                        ...
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                ...
                                <?php foreach ($informasisurat as $value) : ?>
                                    <?php $tanda = Tandatangan::find()->where(['id_informasi' => $value['id']])->one() ?>
                                    <?php $pembuat = Pembuatsurat::find()->where(['id_informasi' => $value['id']])->one() ?>
                                    <?php $tujuan = Tujuansurat::find()->where(['id_informasi_surat' => $value['id']])->one() ?>
                                    <?php if (
                                        Yii::$app->user->identity->role == "admin" || Yii::$app->user->identity->id == $tanda['id_user']
                                    ||  Yii::$app->user->identity->role == "tu"
                                    ) { ?>
                                        <tr align="center">
                                            ...
                                            <td>
                                                <?php $tanda = Tandatangan::find()->where(['id_informasi' => $value["id"]])->one(); ?>
                                                <?php if ($tanda["status"] ==  "diperiksa") { ?>
                                                    <div class="bg-danger"><?php echo $tanda["status"]; ?></div>
                                                <?php } else if ($tanda["status"] == "perbaiki") { ?>
                                                    <div class="bg-warning"><?php echo $tanda["status"]; ?></div>
                                                <?php } ?>
                                                <?php if ($tanda["status"] == "diterima") { ?>
                                                    <div class="bg-success"><?php echo $tanda["status"]; ?></div>
                                                <?php } ?>
                                                <?php if ($tanda["status"] == "ditandatangan") { ?>
                                                    <div class="bg-success"><?php echo $tanda["status"]; ?></div>
                                                <?php } ?>
                                                <?php if ($tanda["status"] == "dikirim") { ?>
                                                            <div class="bg-info"><?php echo $tanda["status"]; ?></div>
                                                        <?php } ?>
                                            </td>
                                            ...
                                            <td align="right">
                                                ...
                                                <?php }if (Yii::$app->user->identity->id == $pembuat["id_user"] && $tanda["status"] == "diperiksa"  || Yii::$app->user->identity->role == "admin") { ?>
